...
public class RestAssuredManager {
	...
	public static Response updateResponse(String httpRequest, String url, HashMap<String, String> usersData) {
		...
		case "post":
			...
			break;
		case "put":
			response = given().param("id", Integer.parseInt(usersData.get("id")))
					.header("Content-type", "application/json").and().body(request.toJSONString()).when().put(url)
					.then().extract().response();
			break;
		case "patch":
			response = given().param("id", Integer.parseInt(usersData.get("id")))
					.header("Content-type", "application/json").and().body(request.toJSONString()).when().patch(url)
					.then().extract().response();
			break;
		}
	...
	public static Response deleteUser(String url, Object... params) {
		Response response = given().param("id", params).contentType(ContentType.JSON).when().delete(url).then()
				.extract().response();
